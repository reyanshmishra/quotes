...
public class DemoImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    public void setImageBitmap(Bitmap bm) {
        if (bm == null) {
            ...
            mImage = new BitmapDrawable(getResources(), bm);
            mImage.setBounds(0, 0, mImage.getIntrinsicWidth(), mImage.getIntrinsicHeight());
            float width_tmp = bm.getWidth(), height_tmp = bm.getHeight();
            float w = getWidth();
            float scale;
...
//        pivotY = mImage.getIntrinsicHeight() / 2;
        ...
        mImage.draw(canvas);
        canvas.restore();
        float h = mImage.getIntrinsicWidth() * mScaleFactor;
